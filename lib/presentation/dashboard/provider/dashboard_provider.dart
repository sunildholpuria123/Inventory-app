...
final totalProductsProvider = StreamProvider<int>((ref) {
  ...
  return db.select(db.products).watch().map((event) => event.length);
});
...
final totalCustomersProvider = StreamProvider<int>((ref) {
  ...
  return db.select(db.customers).watch().map((event) => event.length);
});
...
final totalSalesProvider = StreamProvider<int>((ref) {
  ...
  return db.select(db.invoices).watch().map((event) => event.length);
});
